...
def from_clause(process_metadata, dict_sq):
    ...
    for index, row in table_a_b.iterrows():
        ...
        if table_b_aux != row['table_b']:
            if join_columns.loc[0, 'sq_from_flg_b'] == 'N':
...
def where_clause(process_metadata, dict_sq):
    ...
    dim_columns = process_metadata[['sq_where_flag_a', 'where_value_a', 'where_criteria', 'sq_where_flag_b',
                                    'where_value_b']].drop_duplicates().reset_index(drop=True)
    ...
    for index, row in dim_columns.iterrows():
        if row['sq_where_flag_a'] == 'N':
            if not pd.isnull(row['where_value_a']):
                where_query += (''.join([str(row['where_value_a']), ' ']))
        else:
            where_query += (''.join([str(dict_sq[row['where_value_a']]), ' ']))

        if not pd.isnull(row['where_criteria']):
            where_query += row['where_criteria']
        else:
            return where_query

        if row['sq_where_flag_b'] == 'N':
            if not pd.isnull(row['where_value_b']):
                where_query += (''.join([' ', str(row['where_value_b']), ' ']))
        else:
            where_query += ''.join([' (', str(dict_sq[row['where_value_b']]), ')'])
...
def handler_sq(process_metadata):

    df_sq = process_metadata['subquery_name'].drop_duplicates().dropna().reset_index(drop=True)
    ...
    if len(lista_sq) != 0:
        for item in lista_sq:
            ...
            process_metadata = process_metadata.loc[(process_metadata['subquery_name'] == lista_sq[i])]
            # SELECT
            ...
            # WHERE
            process_metadata_where = process_metadata.loc[(process_metadata['where_value_a'].notnull()
                                                         | process_metadata['where_value_b'].notnull())]\
                .reset_index(drop=True)
